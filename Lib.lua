...
local function ensureKeybindGui()
    if Zilk._keybindGui and Zilk._keybindGui.Parent then
        return Zilk._keybindGui
    end

    local gui = Instance.new("ScreenGui")
    gui.Name = "Zilk_Keybinds"
    gui.ResetOnSpawn = false
    gui.IgnoreGuiInset = true
    gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    gui.Parent = game:GetService("CoreGui")
    ...
    frame.Position = UDim2.new(1, -240, 0.5, -140)
    frame.BackgroundColor3 = Zilk._theme.MainColor
    frame.BorderSizePixel = 0
    frame.Visible = false
    frame.Parent = gui
    makeCorner(frame, 8)
    makeStroke(frame, Zilk._theme.AccentColor)
    registerThemeBinding(frame, "BackgroundColor3", "MainColor")
    ...
    title.Text = "Keybinds"
    title.TextColor3 = Zilk._theme.TextColor
    title.Font = Zilk._theme.Font
    title.TextSize = 14
    title.TextXAlignment = Enum.TextXAlignment.Left
    title.Parent = frame
    registerThemeBinding(title, "TextColor3", "TextColor")
    registerThemeBinding(title, "Font", "Font")
    ...
    Zilk._keybindGui = gui
    return gui
end

local function updateKeybindPanel()
    local gui = ensureKeybindGui()
    ...
    for _, entry in pairs(Zilk._keybindEntries) do
        local label = Instance.new("TextLabel")
        label.BackgroundColor3 = Zilk._theme.SectionColor
        ...
        label.Text = string.format("%s [%s]", entry.name, entry.key)
        label.Parent = list
        makeCorner(label, 4)
        registerThemeBinding(label, "BackgroundColor3", "SectionColor")
...
function Groupbox:CreateKeybind(text, defaultKey, callback, id)
    ...
    record.set = function(v, fromApi)
        ...
        button.Text = name
        Zilk._keybindEntries[id] = { name = text, key = name }
        updateKeybindPanel()
        safeCall(record.changed, record.value)
    end
...
function Zilk:SetKeybindFrameVisible(visible)
    local gui = ensureKeybindGui()
    local frame = gui:FindFirstChild("Frame")
    if frame then
    ...
    for id, value in pairs(data) do
        ...
        if option and typeof(option.SetValue) == "function" then
            ...
            elseif type(value) == "table" and value.__type == "EnumItem" then
                ...
                local itemName = tostring(value.name or "")
                local enumType = enumName:find("UserInputType", 1, true) and Enum.UserInputType or Enum.KeyCode
                local enumValue = enumType[itemName]
                if enumValue then
                    option:SetValue(enumValue)
                end
            else
...
function Zilk:Unload()
    ...
    if self._keybindGui and self._keybindGui.Parent then
