...
-- The ScreenGui uses IgnoreGuiInset = true, so AbsolutePosition is measured from the
-- very top-left of the screen. The deprecated Mouse object excludes the topbar inset (~36px),
-- which throws off every manual hit-test. UserInputService:GetMouseLocation() is inset-aware
-- and lines up with IgnoreGuiInset = true frames, so we use it everywhere for positions.
...
local Library = {
    ...
    UIKeybindTextActiveColor = Color3.fromRGB(147, 112, 219);
    UIKeybindTextInactiveColor = Color3.fromRGB(240, 240, 240);
...
function Library:EnsureCorner(guiObject, Radius)
    ...
    local corner = guiObject:FindFirstChild('HeavN_UICorner')
    if not corner then
        corner = Instance.new('UICorner')
        corner.Name = 'HeavN_UICorner'
...
function Library:RefreshAllCorners()
    ...
    for _, inst in next, Library.CornerInstances do
        if inst and inst.Parent then
            local corner = inst:FindFirstChild('HeavN_UICorner')
            if corner then
                corner.CornerRadius = UDim.new(0, px)
            end
        end
...
function Library:ApplyMainFrameBackground()
    ...
    if self.UIBackgroundImageEnabled and url then
        bg.Image = url
...
function Library:InitAddons(ThemeManager, SaveManager, BuiltInTabs, Options)
    Options = Options or {}
    local folder = Options.Folder or 'HeavNLib'
    ...
    if BuiltInTabs then
        ...
        if SaveManager then BuiltInTabs:SetSaveManager(SaveManager) end
    end
...
function Library:MakeDraggable(Instance, Cutoff)
    ...
    Instance.InputBegan:Connect(function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 then
            local mouseY = GetMouse().Y - Instance.AbsolutePosition.Y
            ...
            dragging = true
            dragStart = Vector2.new(GetMouse().X, GetMouse().Y)
            startPos = Instance.Position

            local dragConnection
            dragConnection = RenderStepped:Connect(function()
                if not dragging or not InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
                    dragging = false
                    dragConnection:Disconnect()
                    return
                end

                local delta = Vector2.new(GetMouse().X - dragStart.X, GetMouse().Y - dragStart.Y)
...
function Library:AddToolTip(InfoStr, HoverInstance)
    ...
    HoverInstance.MouseEnter:Connect(function()
        ...
        Tooltip.Position = UDim2.fromOffset(GetMouse().X + 15, GetMouse().Y + 12)
...
function Library:MouseIsOverOpenedFrame()
    for Frame, _ in next, Library.OpenedFrames do
        ...
        if GetMouse().X >= AbsPos.X and GetMouse().X <= AbsPos.X + AbsSize.X
            and GetMouse().Y >= AbsPos.Y and GetMouse().Y <= AbsPos.Y + AbsSize.Y then
...
function Library:ApplyKeybindPanelStyle()
    ...
    outer.BackgroundColor3 = self.MainColor
    outer.BackgroundTransparency = trans

    if self.KeybindInner then
        self.KeybindInner.BackgroundTransparency = 1
    end

    if self.KeybindStroke then
        self.KeybindStroke.Color = self.AccentColor
        self.KeybindStroke.Transparency = math.clamp(trans * 0.85, 0, 0.75)
    end
...
do
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        -- Apply on focus lost (whether enter pressed or clicked away) — no need to press enter
        ...
        SatVibMap.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    ...
                    local MouseX = math.clamp(GetMouse().X, MinX, MaxX);

                    local MinY = SatVibMap.AbsolutePosition.Y;
                    local MaxY = MinY + SatVibMap.AbsoluteSize.Y;
                    local MouseY = math.clamp(GetMouse().Y, MinY, MaxY);
        ...
        Library:GiveSignal(InputService.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                if GetMouse().X < AbsPos.X or GetMouse().X > AbsPos.X + AbsSize.X
                    or GetMouse().Y < (AbsPos.Y - 20 - 1) or GetMouse().Y > AbsPos.Y + AbsSize.Y then
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        function KeyPicker:Update()
            ...
            ContainerLabel.TextColor3 = State and Library.UIKeybindTextActiveColor or Library.UIKeybindTextInactiveColor;

            if Library.RegistryMap[ContainerLabel] then
                Library.RegistryMap[ContainerLabel].Properties.TextColor3 = function()
                    local active = KeyPicker:GetState()
                    return active and Library.UIKeybindTextActiveColor or Library.UIKeybindTextInactiveColor
                end
            end
        ...
        if not Info.NoUI and Library.RegistryMap[ContainerLabel] then
            Library.RegistryMap[ContainerLabel].Properties.TextColor3 = function()
                return KeyPicker:GetState() and Library.UIKeybindTextActiveColor or Library.UIKeybindTextInactiveColor
            end
        end
...
do
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        -- Always read live (no need to press enter). Save on focus lost to avoid spamming saves.
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local ToggleOuter = Library:Create('TextButton', {
            BackgroundTransparency = 1;
            BorderSizePixel = 0;
        ...
        local ToggleTrack = Library:Create('Frame', {
            BackgroundColor3 = Toggle.Value and Library.AccentColor or Library.ToggleOffColor;
            BorderSizePixel = 0;
        ...
        Library:EnsureCorner(ToggleTrack)

        local ToggleKnob = Library:Create('Frame', {
            BackgroundColor3 = Toggle.Value and Library.ToggleOnColor or Color3.fromRGB(240, 240, 240);
            BorderSizePixel = 0;
        ...
        Library:EnsureCorner(ToggleKnob)
        ...
        function Toggle:Display()
            ToggleTrack.BackgroundColor3 = Toggle.Value and Library.AccentColor or Library.ToggleOffColor;
            ToggleKnob.Position = UDim2.new(Toggle.Value and 1 or 0, Toggle.Value and -16 or 2, 0.5, -7);
            ToggleKnob.BackgroundColor3 = Toggle.Value and Library.ToggleOnColor or Color3.fromRGB(240, 240, 240);
        end;
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        local SliderOuter = Library:Create('Frame', {
            BackgroundTransparency = 1;
            BorderSizePixel = 0;
            Size = UDim2.new(1, 0, 0, Info.Compact and 20 or 45);
            ZIndex = 5;
            Parent = Container;
        });
        ...
        local SliderInner = Library:Create('Frame', {
            ...
            BorderSizePixel = 0;
            Position = UDim2.new(0, 0, 0, Info.Compact and 6 or 25);
            Size = UDim2.new(1, 0, 0, 8);
            ZIndex = 6;
            Parent = SliderOuter;
        });

        Library:EnsureCorner(SliderInner)

        local Fill = Library:Create('Frame', {
            BackgroundColor3 = Library.SliderColor;
            BorderSizePixel = 0;
        ...
        function Slider:UpdateColors()
            Fill.BackgroundColor3 = Library.SliderColor;
            HideBorderRight.BackgroundColor3 = Library.SliderColor;
        end;
        ...
        function Slider:Display()
            ...
            if Info.HideMax then
                text = string.format('%s', Slider.Value .. Suffix)
            else
                -- heavN style: "Text: value"
                text = string.format('%s: %s', Info.Text, Slider.Value .. Suffix);
            end
            ...
            local rel = (Slider.Value - Slider.Min) / math.max(Slider.Max - Slider.Min, 0.001);
            Fill.Size = UDim2.new(math.clamp(rel, 0, 1), 0, 1, 0);
            HideBorderRight.Visible = rel > 0 and rel < 1;
        end;
        ...
        function Slider:GetValueFromXOffset(X)
            local track = math.max(SliderInner.AbsoluteSize.X, 1)
            return Round(Library:MapValue(X, 0, track, Slider.Min, Slider.Max));
        end;
        ...
        SliderInner.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                local trackWidth = math.max(SliderInner.AbsoluteSize.X, 1)
                local function setFromMouse()
                    local rel = math.clamp((GetMouse().X - SliderInner.AbsolutePosition.X) / trackWidth, 0, 1)
                    return Round(Slider.Min + (Slider.Max - Slider.Min) * rel)
                end

                Slider.Value = setFromMouse()
                Slider:Display()
                Library:SafeCallback(Slider.Callback, Slider.Value)
                Library:SafeCallback(Slider.Changed, Slider.Value)

                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    local nValue = setFromMouse()
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        Library:EnsureCorner(DropdownInner)
        ...
        function Dropdown:BuildDropdownList()
            ...
            local Count = 0;
            local LAZY_LOAD_THRESHOLD = 15
            local INITIAL_LOAD_COUNT = 20
            local lazyLoadEnabled = #Values > LAZY_LOAD_THRESHOLD
            local renderedCount = 0
            local allValues = Values
            local loadingMore = false

            -- Lazy load function to add items one by one
            local function loadMoreItems(batchSize)
                ...
                while loaded < batchSize and renderedCount < #allValues do
                    renderedCount = renderedCount + 1
                    local Value = allValues[renderedCount]
                    ...
                    Scrolling.CanvasSize = UDim2.fromOffset(0, renderedCount * 20 + 1)
                    
                    -- Small delay between items to prevent lag
                    if loaded % 5 == 0 then
                        task.wait()
                    end
                end
            ...
            -- Initial load
            if lazyLoadEnabled then
                -- Load initial batch immediately
                loadMoreItems(INITIAL_LOAD_COUNT)
                
                -- Set up scroll detection for lazy loading
                local lastLoadTime = 0
                local scrollConnection
                scrollConnection = Scrolling:GetPropertyChangedSignal('CanvasPosition'):Connect(function()
                    if not ListOuter.Visible then
                        if scrollConnection then scrollConnection:Disconnect() end
                        return
                    end
                    
                    if renderedCount >= #allValues then
                        if scrollConnection then scrollConnection:Disconnect() end
                        return
                    end
                    
                    local scrollPos = Scrolling.CanvasPosition.Y
                    local scrollHeight = Scrolling.CanvasSize.Y.Offset
                    local viewHeight = Scrolling.AbsoluteSize.Y
                    
                    -- Load more when scrolling near bottom (loads 5 items at a time)
                    if scrollPos + viewHeight >= scrollHeight - 30 then
                        local now = tick()
                        if now - lastLoadTime > 0.05 then
                            lastLoadTime = now
                            task.spawn(function()
                                loadMoreItems(5)
                            end)
                ...
                -- Also trigger load when dropdown opens
                local openConnection
                openConnection = ListOuter:GetPropertyChangedSignal('Visible'):Connect(function()
                    if ListOuter.Visible and renderedCount < #allValues then
                        task.spawn(function()
                            loadMoreItems(10)
                        end)
                    end
                ...
                -- Load all items immediately for small lists
                Values = allValues
            end

            if not lazyLoadEnabled then
                for Idx, Value in next, Values do
                local Table = {};
            ...
            end -- Close if not lazyLoadEnabled
...
do
    ...
    Library.KeybindStroke = KeybindStroke;
...
-- heavN-style notification. Supports:
--   Library:Notify('message')                       -> message only
--   Library:Notify('message', 5)                    -> message + time
--   Library:Notify('Title', 'message')              -> title + message
--   Library:Notify('Title', 'message', 5)           -> title + message + time
--   Library:Notify('Title', 'message', 5, color)    -> + accent override
function Library:Notify(a, b, c, d)
    ...
    Library:EnsureCorner(NotifyOuter)
...
function Library:CreateWindow(...)
    ...
    local TitleBar = Library:Create('Frame', {
        ...
        BorderSizePixel = 0;
        Size = UDim2.new(1, 0, 0, TITLE_BAR_H);
        ZIndex = 2;
        Parent = Inner;
    });
    ...
    ResizeHandle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            resizing = true
            resizeStart = Vector2.new(GetMouse().X, GetMouse().Y)
            startSize = Outer.Size

            local resizeConnection
            resizeConnection = RenderStepped:Connect(function()
                if not resizing or not InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
                    resizing = false
                ...
                local delta = Vector2.new(GetMouse().X - resizeStart.X, GetMouse().Y - resizeStart.Y)
    ...
    local TabArea = Library:Create('ScrollingFrame', {
        ...
        Size = UDim2.new(0, 110, 1, -TAB_TOP);
        CanvasSize = UDim2.new(0, 0, 0, 0);
    ...
    local TabContainer = Library:Create('Frame', {
        BackgroundTransparency = 1;
        Position = UDim2.new(0, 115, 0, TAB_TOP);
        Size = UDim2.new(1, -120, 1, -(TAB_TOP + 10));
        ZIndex = 2;
        Parent = Inner;
    });
    ...
    function Window:SetTabPosition(pos)
        ...
        if pos == 'Topbar' then
            TabArea.Position = UDim2.new(0, 0, 0, TAB_TOP);
            TabArea.Size = UDim2.new(1, 0, 0, 32);
            ...
            TabContainer.Position = UDim2.new(0, 8, 0, TAB_TOP + 36);
            TabContainer.Size = UDim2.new(1, -16, 1, -(TAB_TOP + 46));
            ...
            TabArea.Position = UDim2.new(0, 0, 0, TAB_TOP);
            TabArea.Size = UDim2.new(0, 110, 1, -TAB_TOP);
            ...
            TabContainer.Position = UDim2.new(0, 115, 0, TAB_TOP);
            TabContainer.Size = UDim2.new(1, -120, 1, -(TAB_TOP + 10));
    ...
    function Window:AddTab(Name)
        ...
        Library:EnsureCorner(TabButton)

        table.insert(Window._TabButtons, TabButton);
        -- Apply current tab layout sizing to the new button
        if Window._TabPosition == 'Topbar' then
            TabButton.AutomaticSize = Enum.AutomaticSize.X;
            TabButton.Size = UDim2.new(0, 0, 1, -6);
            local pad = TabButton:FindFirstChildOfClass('UIPadding');
            if pad then
                pad.PaddingLeft = UDim.new(0, 14);
                pad.PaddingRight = UDim.new(0, 14);
            end
        ...
        function Tab:HideTab()
            TabButton.BackgroundColor3 = Library.SectionColor;
            Library.RegistryMap[TabButton].Properties.BackgroundColor3 = 'SectionColor';
        ...
        function Tab:AddGroupbox(Info)
            ...
            GroupboxLabel.TextColor3 = Library.AccentColor;
            ...
            function Groupbox:Resize()
                ...
                BoxOuter.Size = UDim2.new(1, 0, 0, 34 + Size + 10);
            end;
    ...
    function Library:Toggle()
        ...
        -- Close all opened frames (dropdowns, color pickers) when closing menu
