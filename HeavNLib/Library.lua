...
function Library:InitAddons(ThemeManager, SaveManager, BuiltInTabs, Options)
    ...
    -- Theme folder is set in code (like the config folder). Themes are stored in <themeFolder>/themes.
    local themeFolder = Options.ThemeFolder or folder
...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        ToggleOuter.MouseButton1Click:Connect(function()
            ...
            -- Only react when the click is actually on the label text or the switch,
            -- not the empty gap in the middle/sides of the row.
            local rowLeft = ToggleOuter.AbsolutePosition.X
            local rowWidth = ToggleOuter.AbsoluteSize.X
            local mx = Mouse.X

            local labelRight = rowLeft + math.min((ToggleLabel.TextBounds.X + 8), rowWidth)
            local switchLeft = rowLeft + rowWidth - 45

            local overLabel = mx >= rowLeft and mx <= labelRight
            local overSwitch = mx >= switchLeft and mx <= (rowLeft + rowWidth)

            if overLabel or overSwitch then
        ...
        SliderInner.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and not Library:MouseIsOverOpenedFrame() then
                ...
                local function setFromMouse()
                    local rel = math.clamp((Mouse.X - SliderInner.AbsolutePosition.X) / trackWidth, 0, 1)
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        InputService.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                -- Don't close if the click is on the dropdown button itself (it handles open/close)
    ...
    ResizeHandle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            resizeConnection = RenderStepped:Connect(function()
                ...
                local delta = Vector2.new(Mouse.X - resizeStart.X, Mouse.Y - resizeStart.Y)
