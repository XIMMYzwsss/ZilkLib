...
-- Ignore Settings UI elements from being saved into configs
...
local Window = Library:CreateWindow({
	...
	Center = true,
	AutoShow = true,
	BuiltInTabs = true,  -- Auto-creates the Settings tab (configs live inside it)
	TabPadding = 5,
	MenuFadeTime = 0.15,
})
...
-- Textboxes read live now (no need to press Enter) and never auto-clear on click
...
SpecialBox:AddButton({
	...
	Func = function()
		Library:Notify('HeavNLib', 'This is a notification with a title and body.\nNotifications appear top-right.', 5)
	end,
})
...
--  Demonstrates the two main keybind patterns:
--   1) Aimbot  -> keybind ONLY works while its toggle is ON (toggle-gated)
--   2) Fly     -> NO toggle, the keybind works on its own at any time
-- ═══════════════════════════════════════════════════════
local Tab4 = Window:AddTab('Practical Examples')
local RunService = game:GetService('RunService')

-- Explanation groupbox
...
HowToBox:AddDivider()
HowToBox:AddLabel('2) Fly has NO toggle. Its keybind works by itself at any time - press it and Fly turns on/off.', true)

-- ── Aimbot: toggle-gated keybind ───────────────────────
...
AimbotBox:AddLabel('Aimbot Key'):AddKeyPicker('AimbotKey', {
	Default = 'E',   -- unique key so it doesn't clash with other demos
	Mode = 'Hold',
	Text = 'Aimbot',
})
...
-- Extra options only appear once the toggle is enabled
...
aimbotEnabled:OnChanged(function()
	aimbotStatus:SetText(aimbotEnabled.Value and 'Status: ON - hold the key to aim' or 'Status: toggle OFF - key ignored')
end)

-- Behaviour: the key ONLY does something while the toggle is enabled.
...
local AimbotConnection = RunService.RenderStepped:Connect(function()
	...
	if down and not aimWasDown then
		if aimbotEnabled.Value then
			Library:Notify('Aimbot', 'Locking onto target... (toggle is ON)', 1)
			-- real aimbot logic would run here
		else
			Library:Notify('Aimbot', 'Key pressed but toggle is OFF - ignoring.', 2)
		end
	end
	-- While the toggle is off, the key does nothing no matter what.
	aimWasDown = down
end)

-- ── Fly: keybind only, no toggle ───────────────────────
...
-- Behaviour: the key works on its own, no toggle gating.
...
local FlyConnection = RunService.RenderStepped:Connect(function()
	...
	if on ~= flyWasOn then
		flyWasOn = on
		Library:Notify('Fly', on and 'Fly enabled (key only)' or 'Fly disabled', 2)
		-- real fly logic would start/stop here
	end
end)

-- ── ESP example (master toggle + dependent options) ────
...
Library:Notify('HeavNLib Demo', 'Loaded successfully!\nExplore all tabs to see features.\n\nPress RightShift to toggle menu.\nDrag bottom-right corner to resize.\n\nSettings tab now holds Themes + Configs.\nTry the Practical Examples tab to see\nhow toggle-gated vs keybind-only features work!', 9)
