...
local BuiltInTabs = {} do
	...
	function BuiltInTabs:BuildSettingsTab(tab, config)
		...
		local menuKey = ui:AddLabel('Menu bind'):AddKeyPicker('MenuKeybind', {
			Default = 'RightShift',
			NoUI = true,
			Mode = 'Hold',
			Text = 'Menu keybind',
		})
		lib:SetToggleKeybind(menuKey or opt('MenuKeybind'))
		if menuKey then
		...
		if self.ThemeManager then
			...
			local accentPicker = ui:AddLabel('Accent'):AddColorPicker('AccentColor', { Default = lib.AccentColor })
			...
			local fontPicker = ui:AddLabel('GUI text'):AddColorPicker('FontColor', { Default = lib.FontColor })
