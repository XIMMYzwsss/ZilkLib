...
local BuiltInTabs = {} do
	...
	function BuiltInTabs:BuildSettingsTab(tab, config)
		...
		-- Configs live in the Settings tab (left side, below UI Settings)
		if self.SaveManager then
			self.SaveManager:BuildConfigSection(tab)
		end

		-- Themes (right side) — appearance options + a separate save/load box
	...
	function BuiltInTabs:Attach(window, config)
		...
		-- Configs are merged into the Settings tab (no separate Configs tab)
