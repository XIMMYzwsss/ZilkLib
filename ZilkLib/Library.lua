...
function Library:HasCustomBackground()
    return self.UIBackgroundImageEnabled == true
        and self:ResolveGuiBackgroundImageUrl(self.UIBackgroundImageAssetId or '') ~= nil
end;

function Library:GetChromeBackgroundTransparency()
    return self:HasCustomBackground() and 1 or 0
end;

function Library:RefreshTabButtonStyles()
    if not self.Window or not self.Window.Tabs then return end

    local custom = self:HasCustomBackground()
    for _, tab in next, self.Window.Tabs do
        local btn = tab.Button
        local frame = tab.TabFrame
        if not btn then continue end

        local isActive = frame and frame.Visible
        if custom then
            btn.BackgroundTransparency = 1
            btn.BorderColor3 = isActive and self.AccentColor or self.OutlineColor
        else
            btn.BackgroundTransparency = 0
            btn.BackgroundColor3 = isActive and self.MainColor or self.BackgroundColor
            btn.BorderColor3 = self.OutlineColor
        end
    end
end;

function Library:ApplyCustomBackgroundChrome()
    for _, Object in next, self.Registry do
        local prop = Object.Properties.BackgroundTransparency
        if type(prop) == 'function' then
            Object.Instance.BackgroundTransparency = prop()
...
function Library:ApplyMainFrameBackground()
    ...
    self:ApplyCustomBackgroundChrome()
end;
...
function Library:CreateWindow(...)
    ...
    Library:AddToRegistry(TitleBar, {
        ...
        BackgroundTransparency = function()
            return Library:GetChromeBackgroundTransparency()
        end;
    });
        ...
        function Tab:ShowTab()
            for _, OtherTab in next, Window.Tabs do
                if OtherTab.TabFrame then
                    OtherTab.TabFrame.Visible = OtherTab == Tab
                end
            end

            Library:RefreshTabButtonStyles()
        end;
