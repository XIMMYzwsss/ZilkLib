...
-- RIVALS universe: CoreGui ScreenGui trees get scanned hard and CreateWindow hard-locks the client.
local IS_RIVALS = game.GameId == 6035872082;

local ScreenGui = Instance.new('ScreenGui');
ProtectGui(ScreenGui);

ScreenGui.Name = IS_RIVALS and ('Zilk_' .. tostring(math.random(1e6, 9e6))) or 'ZilkLib';
ScreenGui.ZIndexBehavior = IS_RIVALS and Enum.ZIndexBehavior.Sibling or Enum.ZIndexBehavior.Global;
ScreenGui.IgnoreGuiInset = true;
ScreenGui.ResetOnSpawn = false;
if IS_RIVALS then
    -- Sit above MainGui / other PlayerGui menus
...
function Library:_BindRivalsMouseUnlock()
    if self._RivalsMouseBound then
        return;
    end
    self._RivalsMouseBound = true;
    -- After CameraController (runs every frame) so we win the mouse lock fight.
    RunService:BindToRenderStep('ZilkLib_RivalsMouse', Enum.RenderPriority.Last.Value + 1, function()
...
function Library:_UnbindRivalsMouseUnlock()
    if not self._RivalsMouseBound then
        return;
    end
    self._RivalsMouseBound = false;
    pcall(function()
        RunService:UnbindFromRenderStep('ZilkLib_RivalsMouse');
    end)
end

function Library:Unload()
    Library:_UnbindRivalsMouseUnlock();
...
function Library:CreateWindow(...)
    ...
    function Library:Toggle()
        ...
        if self._MenuModal and not IS_RIVALS then
        ...
        if self._MenuToggled then
            ...
            if IS_RIVALS then
                Library:_BindRivalsMouseUnlock();
            else
                -- RIVALS CameraController owns mouse every frame; Drawing cursor fight hard-locks.
                task.spawn(function()
