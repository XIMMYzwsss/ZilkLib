...
function Library:RegisterPanelSurface(guiObject)
    if not guiObject then return end
    table.insert(self.PanelSurfaces, guiObject)
    local clear = self:IsCustomBackgroundActive()
    self:SetSurfaceTransparency(guiObject, clear and 1 or 0)
end;

function Library:IsCustomBackgroundActive()
    if not self.UIBackgroundImageEnabled then return false end
    return self:ResolveGuiBackgroundImageUrl(self.UIBackgroundImageAssetId or '') ~= nil
end;

function Library:SetSurfaceTransparency(inst, trans)
    if not inst or not inst.Parent then return end
    if not (inst:IsA('GuiObject')) then return end
    pcall(function()
        inst.BackgroundTransparency = trans
    end)
    local cache = self._MenuTransparencyCache and self._MenuTransparencyCache[inst]
    if cache then
        cache.BackgroundTransparency = trans
    end
end;
...
function Library:ApplyMainFrameBackground()
    ...
    local clear = self.UIBackgroundImageEnabled and url ~= nil
    ...
    if win then
        self:SetSurfaceTransparency(win.TitleBar, surfaceTrans)
        self:SetSurfaceTransparency(win._TabArea, surfaceTrans)
        self:SetSurfaceTransparency(win._TabContainer, surfaceTrans)
        if win._TabButtons then
            for _, btn in next, win._TabButtons do
                -- Keep selected tab readable; clear fill on inactive tab buttons so the image shows through.
                local isActive = win.ActiveTab and win.ActiveTab.Button == btn
                self:SetSurfaceTransparency(btn, (clear and not isActive) and 1 or 0)
            end
        end
    end

    for i = #self.PanelSurfaces, 1, -1 do
        local frame = self.PanelSurfaces[i]
        if not frame or not frame.Parent then
            table.remove(self.PanelSurfaces, i)
        else
            self:SetSurfaceTransparency(frame, surfaceTrans)
        end
    end
end;
...
function Library:CreateWindow(...)
    ...
    function Window:AddTab(Name, Info)
        ...
        function Tab:ShowTab()
            ...
            if Library.IsCustomBackgroundActive and Library:IsCustomBackgroundActive() then
                Library:SetSurfaceTransparency(TabButton, 0);
            end
        end;
        ...
        function Tab:HideTab()
            ...
            if Library.IsCustomBackgroundActive and Library:IsCustomBackgroundActive() then
                Library:SetSurfaceTransparency(TabButton, 1);
            end
        end;
        ...
        function Tab:AddGroupbox(Info)
            ...
            Library:RegisterPanelSurface(BoxOuter);
            ...
            Library:RegisterPanelSurface(BoxInner);
