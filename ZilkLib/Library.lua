...
local Library = {
    ...
    -- RIVALS (and similar) CameraController rewrites MouseIconEnabled/MouseBehavior every frame.
    -- SafeInput skips the Drawing cursor loop that fights that and crashes the client.
    SafeInput = false;
    ForceDefaultMouseWhileOpen = false;
    _RivalsMouseBound = false;
...
do
    ...
    if game.GameId == 6035872082 then
        Library.SafeInput = true;
        Library.ForceDefaultMouseWhileOpen = true;
    end
end
...
function Library:EnsureCorner(guiObject, Radius)
    ...
    -- UICorner kills BorderSizePixel bevels/borders even at radius 0 — only keep it when needed.
    if px <= 0 then
...
function Library:ApplySurfaceOutline(inst, enabled)
    if not inst or not inst:IsA('GuiObject') then return end
    local stroke = inst:FindFirstChild('Zilk_SurfaceStroke')
    if enabled then
        if not stroke then
            stroke = Instance.new('UIStroke')
            stroke.Name = 'Zilk_SurfaceStroke'
        ...
        stroke.Enabled = true
        -- Prefer UIStroke so the panel edge stays visible with a transparent fill.
        inst.BorderSizePixel = 0
    else
        if stroke then
            stroke.Enabled = false
        end
        if inst.BorderColor3 then
...
function Library:ApplyMainFrameBackground()
    ...
    -- Strip any leftover UICorners that would hide the accent BorderSizePixel rim.
    for _, inst in next, { outer, inner, bd, bg } do
        if inst then
            local leftover = inst:FindFirstChild('Zilk_UICorner')
    ...
    if clear then
        ...
        if inner then
            -- Keep Inner opaque enough for BorderSizePixel accent rim to render;
            -- the image sits above the backdrop either way.
    ...
    -- Accent rim around the whole menu (Inner BorderColor3 = AccentColor).
    -- When Inner is transparent, BorderSizePixel can disappear — paint it with UIStroke.
    if inner then
        local rim = inner:FindFirstChild('Zilk_AccentRim')
        if clear then
            if not rim then
                rim = Instance.new('UIStroke')
                rim.Name = 'Zilk_AccentRim'
                rim.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
                rim.LineJoinMode = Enum.LineJoinMode.Miter
                rim.Thickness = 1
                rim.Parent = inner
            end
            rim.Color = self.AccentColor
            rim.Enabled = true
        elseif rim then
            rim.Enabled = false
        end
    ...
    -- Clear fills behind groupboxes / tab list; keep outlines so regions stay readable.
    local surfaceTrans = clear and 1 or 0
    for _, inst in next, self.BgClearSurfaces do
        if inst and inst.Parent then
            inst.BackgroundTransparency = surfaceTrans
            self:ApplySurfaceOutline(inst, clear and true or false)
...
function Library:_BindSafeMouseOverride()
    if self._RivalsMouseBound then
        return;
    end
    self._RivalsMouseBound = true;
    RunService:BindToRenderStep('ZilkLib_SafeMouse', Enum.RenderPriority.Last.Value + 1, function()
        if Library._MenuToggled then
...
function Library:_UnbindSafeMouseOverride()
    if not self._RivalsMouseBound then
        return;
    end
    self._RivalsMouseBound = false;
    pcall(function()
        RunService:UnbindFromRenderStep('ZilkLib_SafeMouse');
    end)
end

function Library:Unload()
    Library:_UnbindSafeMouseOverride();
...
do
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        function Dropdown:RefreshButtonStates()
            local buttons = Dropdown.Buttons
            if type(buttons) ~= 'table' then return end
            for _, Table in next, buttons do
                if type(Table.UpdateButton) == 'function' then
                    Table:UpdateButton()
                end
            end
        end

        function Dropdown:SetValues(NewValues, Rebuild)
            if NewValues then
                if not Rebuild and valuesEqual(Dropdown.Values, NewValues) then
                    Dropdown:Display()
                    return
                end
            ...
            -- Lazy: only rebuild the scrolling list when open (or forced).
            if Rebuild == true or ListOuter.Visible then
                Dropdown:BuildDropdownList();
            else
                Dropdown._listDirty = true;
                Dropdown:Display();
            end
        end;

        function Dropdown:OpenDropdown()
            if Dropdown._listDirty ~= false or not Dropdown.Buttons then
                Dropdown:BuildDropdownList();
            end
            RecalculateListPosition();
            RecalculateListSize(ListOuter.Size.Y.Offset);
        ...
        function Dropdown:SetValue(Val, Fire)
            ...
            -- Avoid full list rebuild on every programmatic SetValue (was the gun-switch hitch).
            if ListOuter.Visible then
                if Dropdown._listDirty then
                    Dropdown:BuildDropdownList();
                else
...
function Library:CreateWindow(...)
    ...
    -- Optional per-window overrides (RIVALS auto-enables these via GameId)
    if Config.SafeInput ~= nil then Library.SafeInput = Config.SafeInput == true end
    if Config.ForceDefaultMouseWhileOpen ~= nil then
        Library.ForceDefaultMouseWhileOpen = Config.ForceDefaultMouseWhileOpen == true
    end
    ...
    -- Do not put UICorner on Outer/Inner — it removes the accent BorderSizePixel rim.
    ...
    function Library:Toggle()
        ...
        if self._MenuToggled then
            ...
            if Library.ForceDefaultMouseWhileOpen then
                Library:_BindSafeMouseOverride();
            end

            -- SafeInput (RIVALS): skip Drawing cursor + MouseIconEnabled=false spam.
            -- That loop fights CameraController and hard-crashes the client.
            if not Library.SafeInput then
                task.spawn(function()
