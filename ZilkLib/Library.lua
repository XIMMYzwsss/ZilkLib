...
function Library:IsCustomBackgroundActive()
    return self.UIBackgroundImageEnabled == true
        and self:ResolveGuiBackgroundImageUrl(self.UIBackgroundImageAssetId or '') ~= nil
end;

function Library:SyncMenuTransparencyCache(inst, prop, value)
    local cacheMap = self._MenuTransparencyCache
    if not cacheMap or not inst then return end
    local cache = cacheMap[inst]
    if cache then
        cache[prop] = value
    end
end;

function Library:SetBackgroundPanelTransparency(inst, trans)
    if not inst or not inst.Parent then return end
    inst.BackgroundTransparency = trans
    self:SyncMenuTransparencyCache(inst, 'BackgroundTransparency', trans)
end;

function Library:RegisterBackgroundPanel(inst)
    if not inst then return end
    self.BackgroundPanels = self.BackgroundPanels or {}
    table.insert(self.BackgroundPanels, inst)
    if self:IsCustomBackgroundActive() then
        self:SetBackgroundPanelTransparency(inst, 1)
    end
end;
...
function Library:ApplyMainFrameBackground()
    ...
    local active = self.UIBackgroundImageEnabled and url ~= nil
    local chromeTrans = active and 1 or 0
    ...
    if active then
        ...
        bg.Visible = true
        self:SetBackgroundPanelTransparency(bd, 1)
        if inner then
            self:SetBackgroundPanelTransparency(inner, 1)
        end
        self:SyncMenuTransparencyCache(bg, 'ImageTransparency', 0)
        self:SyncMenuTransparencyCache(bg, 'BackgroundTransparency', 1)
        ...
        self:SetBackgroundPanelTransparency(bd, 0)
        if inner then
            inner.BackgroundColor3 = self.MainColor
            self:SetBackgroundPanelTransparency(inner, 0)
        end
        self:SyncMenuTransparencyCache(bg, 'ImageTransparency', 1)
    end

    -- Hide solid chrome behind tabs / groupboxes so the image shows through.
    for _, panel in next, self.BackgroundPanels or {} do
        if panel and panel.Parent then
            self:SetBackgroundPanelTransparency(panel, chromeTrans)
        end
    end
end;
...
function Library:CreateWindow(...)
    ...
    Library:RegisterBackgroundPanel(TitleBar);
    ...
    Library:RegisterBackgroundPanel(TabArea);
    ...
    Library:RegisterBackgroundPanel(TabContainer);
    ...
    function Window:AddTab(Name, Info)
        ...
        function Tab:AddGroupbox(Info)
            ...
            Library:RegisterBackgroundPanel(BoxOuter);
            ...
            Library:RegisterBackgroundPanel(BoxInner);
