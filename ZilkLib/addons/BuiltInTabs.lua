local BuiltInTabs = {} do
	...
	function BuiltInTabs:BuildSettingsTab(tab, config)
		...
		showKeybinds:OnChanged(applyUISettings)
		if config and config.Watermark ~= false then
			wmToggle = ui:AddToggle('Settings_Watermark', {
				Text = 'Show watermark',
				Default = true,
			})
			wmToggle:OnChanged(applyUISettings)
		end
		self._applyUISettings = applyUISettings
		applyUISettings()
	...
	function BuiltInTabs:Attach(window, config)
		...
		window._SettingsTabName = settingsName
		local settingsTab = window:AddTab(settingsName, { IsSettings = true })
		...
		if settingsTab.Button then
			settingsTab.Button.LayoutOrder = 99999
		end
