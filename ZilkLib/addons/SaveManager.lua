...
local SaveManager = {} do
	...
	SaveManager.Parser = {
		Toggle = {
			...
			Load = function(idx, data)
				local t = (rawget(getgenv(), 'Toggles') or Toggles)
				if t and t[idx] then t[idx]:SetValue(data.value) end
			end,
		},
		Slider = {
			Save = function(idx, object)
			...
			Load = function(idx, data)
				local o = (rawget(getgenv(), 'Options') or Options)
				if o and o[idx] then o[idx]:SetValue(data.value) end
			end,
		},
		Dropdown = {
			Save = function(idx, object)
		...
		ColorPicker = {
			...
			Load = function(idx, data)
				local o = (rawget(getgenv(), 'Options') or Options)
				if o and o[idx] then o[idx]:SetValueRGB(Color3.fromHex(data.value), data.transparency) end
			end,
		},
		KeyPicker = {
			Save = function(idx, object)
			...
			Load = function(idx, data)
				local o = (rawget(getgenv(), 'Options') or Options)
				if o and o[idx] then o[idx]:SetValue({ data.key, data.mode }) end
			end,
		},
		Input = {
			Save = function(idx, object)
			...
			Load = function(idx, data)
				local o = (rawget(getgenv(), 'Options') or Options)
				if o and o[idx] and type(data.text) == 'string' then o[idx]:SetValue(data.text) end
			end,
		},
	}
	...
	function SaveManager:BuildFolderTree()
		-- Create parents first. Avoid string.split (missing on some executors).
		local parts = {}
		for part in string.gmatch(tostring(self.Folder or ''), '[^/\\]+') do
			parts[#parts + 1] = part
		end
		local path = ''
		for idx = 1, #parts do
			path = (idx == 1) and parts[idx] or (path .. '/' .. parts[idx])
	...
	function SaveManager:Save(name)
		...
		if typeof(writefile) ~= 'function' then return false, 'executor missing writefile' end
		self:BuildFolderTree()
		local fullPath = self.Folder .. '/' .. name .. '.json'
		local data = { objects = {} }
		local toggles = rawget(getgenv(), 'Toggles') or Toggles or {}
		local options = rawget(getgenv(), 'Options') or Options or {}
		for idx, toggle in next, toggles do
			if not self.Ignore[idx] and toggle and self.Parser[toggle.Type] then
				table.insert(data.objects, self.Parser[toggle.Type].Save(idx, toggle))
			end
		end
		for idx, option in next, options do
			if not self.Ignore[idx] and option and self.Parser[option.Type] then
	...
	function SaveManager:LoadAutoloadConfig()
		...
		if isfile(autoloadPath) then
			local name = tostring(readfile(autoloadPath) or ''):gsub('[%s%z\r\n]+$', '')
	...
	function SaveManager:Confirm(title, message, onConfirm)
		...
		-- Outer black rim + inset outline (same Linoria 3D as main window / AddButton)
		...
		-- Only applies when UICornerRadius > 0 (radius 0 removes UICorner so borders stay).
		...
		local ttl = Lib:Create('TextLabel', {
		...
		local function makeDialogButton(text, position)
			-- Exact AddButton bevel: black Outer + inset Outline Inner (20px tall like GUI buttons).
			...
			Lib:AddToRegistry(label, { TextColor3 = 'FontColor' })
	...
	function SaveManager:BuildConfigSection(tab)
		...
		section:AddDivider()
		local function opts()
			return rawget(getgenv(), 'Options') or Options
		end
		section:AddButton('Create config', function()
			local o = opts()
			local name = o and o.SaveManager_ConfigName and o.SaveManager_ConfigName.Value
			if not name or name:gsub(' ', '') == '' then return self.Library:Notify('Invalid config name (empty)', 2) end
			self:Confirm('Save config', 'Save config "' .. name .. '"?', function()
				...
				if o.SaveManager_ConfigList then
			...
			local o = opts()
			local name = (o.SaveManager_ConfigList and o.SaveManager_ConfigList.Value) or (o.SaveManager_ConfigName and o.SaveManager_ConfigName.Value)
		...
		section:AddButton('Overwrite config', function()
			local o = opts()
			local name = o and o.SaveManager_ConfigList and o.SaveManager_ConfigList.Value
			...
			self:Confirm('Delete config', 'Delete "' .. name .. '" permanently?', function()
				...
				pcall(function()
					if isfile(self.Folder .. '/autoload.txt') then
						auto = readfile(self.Folder .. '/autoload.txt')
					end
				end)
				if self.AutoloadLabel then self.AutoloadLabel:SetText('Current autoload config: ' .. tostring(auto)) end
			end)
		end)
		section:AddButton('Refresh list', function()
			local o = opts()
			if o and o.SaveManager_ConfigList then
				o.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
				o.SaveManager_ConfigList:SetValue(nil)
			end
		end)
		section:AddButton('Set as autoload', function()
			...
			self:BuildFolderTree()
			local ok, err = pcall(writefile, self.Folder .. '/autoload.txt', name)
			if not ok then
