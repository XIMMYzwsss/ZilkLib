...
local SaveManager = {} do
	...
	function SaveManager:Confirm(title, message, onConfirm)
		...
		local function createDialogButton(parent, text, position, size)
			...
			inner.BackgroundColor3 = Library.ButtonColor
		...
		panelOuter.ClipsDescendants = true
		panelOuter.ZIndex = 200001
		panelOuter.Parent = overlay
		Library:EnsureCorner(panelOuter)

		local panel = Instance.new('Frame')
		panel.Size = UDim2.new(1, 0, 1, 0)
		panel.BackgroundColor3 = Library.MainColor
		panel.BorderColor3 = Library.OutlineColor
		...
		ttl.Font = Library.Font
		ttl.TextSize = 16
		ttl.TextColor3 = Library.AccentColor
		...
		body.TextSize = 14
		body.TextColor3 = Library.FontColor
		...
		row.Position = UDim2.new(0, 8, 1, -32)
		row.ZIndex = 200003

		local cancel = createDialogButton(row, 'Cancel', UDim2.new(0, 0, 0, 0), UDim2.new(0.48, 0, 1, 0))
		local ok = createDialogButton(row, 'Confirm', UDim2.new(0.52, 0, 0, 0), UDim2.new(0.48, 0, 1, 0))

		local function close()
			if overlay.Parent then overlay:Destroy() end
		end

		cancel.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 then
				close()
			end
		end)
		ok.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 then
				close()
				pcall(onConfirm)
			end
